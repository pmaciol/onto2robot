...
class SideBottom2Twist:
    ...
    def update_and_evaluate(self, side_sensors: list[float], bottom_sensors: list[float]) -> Twist:
        if len(side_sensors) != self.side_sensors_count or len(bottom_sensors) != self.bottom_sensors_count:
            print(
                f"Invalid sensor data length. Expected {self.side_sensors_count} "
                f"side sensors and {self.bottom_sensors_count} bottom sensors."
            )
            return Twist()
        direction = self.reasoner(side_sensors, bottom_sensors)
